...
const KaranJohar = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let res = await axios.get("src/Json/KaranJohar.json");
        setData(res.data);
      } catch (error) {
        console.log(error);
      }
  ...
  return (
    <>
      ...
      <MiddleNavbar />
      <DropdownNavbar />
      <Box><img src=" https://static1.lenskart.com/media/desktop/img/Jan23/23jan/karan-pid-pdp/PLP.jpg" alt="" /></Box>
      <Flex
        ...
        <SimpleGrid gridTemplateColumns={"repeat(3,1fr)"} ml={10} gap={5} w={"60%"} mt={6}>
          {sortedData.map((elem) => (
            <SimpleGrid  key={elem.id} cursor={"pointer"} borderRadius={10} w={300} boxShadow="xs" rounded="md">
              <Link to={`/karanjoharproduct/${elem.id}`}> 
