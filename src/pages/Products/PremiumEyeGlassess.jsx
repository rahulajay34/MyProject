...
const Premiumproduct = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let res = await axios.get("src/Json/PREMIUMEYEGLASSES.json ");
        setData(res.data);
      } catch (error) {
        console.log(error);
      }
  ...
  return (
    <>
      ...
      <MiddleNavbar />
      <DropdownNavbar />
      <Box><img src=" https://static1.lenskart.com/media/desktop/img/2024/apr/vibe-badlo/desktop-plp-jj-eye.png" alt="" /></Box>
      <Flex
        ...
        <SimpleGrid gridTemplateColumns={"repeat(3,1fr)"} ml={10} gap={5} w={"60%"} mt={6}>
          {sortedData.map((elem) => (
            <SimpleGrid  key={elem.id} cursor={"pointer"} borderRadius={10} w={300} boxShadow="xs" rounded="md">
              <Link to={`/premuimproduct/${elem.id}`}> 
